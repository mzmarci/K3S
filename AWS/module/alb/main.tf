...
resource "aws_lb_target_group" "eks_asg_tg" {
...
resource "aws_lb_listener_rule" "asg_listener" {
  ...
  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.eks_asg_tg.arn
  }
  condition {
    path_pattern {
      values = ["/api"]
...
resource "aws_autoscaling_group" "eks_autoscaling" {
  ...
  launch_template {
    id      = aws_launch_template.eks_asg.id
    version = aws_launch_template.eks_asg.latest_version
  }

  target_group_arns = [aws_lb_target_group.eks_asg_tg.arn]
}
...
resource "aws_autoscaling_policy" "scale_up" {
  ...
  autoscaling_group_name = aws_autoscaling_group.eks_autoscaling.name
}
